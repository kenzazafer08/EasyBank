...
public class AccountDAO implements AccountI {
        ...
        try (Connection connection = dbConnection.getConnection()) {
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                ...
                while (resultSet.next()) {
                    ...
                    String client_code = resultSet.getString("account_client_code");
                    ClientDAO clientDAO = new ClientDAO(dbConnection);
                    Client client = clientDAO.searchByCode(client_code);
    ...
    @Override
    public Account searchByOperationN(String n) {
        ...
        try (Connection connection = dbConnection.getConnection()) {
            String query = "SELECT " +
                    ...
                    "LEFT JOIN currentaccount AS c ON a.number = c.account_number " +
                    "LEFT JOIN savingaccount AS s ON a.number = s.account_number " +
                    "INNER JOIN operation AS o ON a.number = o.account_number " +
                    "WHERE " +
                    "o.number = ?";
